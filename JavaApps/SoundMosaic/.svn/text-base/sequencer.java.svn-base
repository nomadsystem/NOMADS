...
public class sequencer extends JApplet implements ActionListener, Runnable
{
    // will always need this
    private Socket socket              = null;
    private DataInputStream  streamIn   = null;
    private DataOutputStream streamOut = null;
    private sequencer_thread client  = null;
   ...
   private static void addAButton(String text, Container container) {
	...
	try {
	    ...
	    client = new sequencer_thread(this, socket);
	    streamOut.writeByte(app_id.STUDENT_SEQUENCER);
	    streamOut.writeUTF("");
 	}
	catch(IOException ioe) {
	    hPrint("Error opening output stream: ");
	} 
    }
	...
	// START/STOP CONTROL
	else if (s.equals("SOOCON")) {
	    SOOC = true;
	    start.setEnabled(false);
	}
	else if (s.equals("SOOCOFF")) {
	    SOOC = false;
	    start.setEnabled(false);
	}
	// TEMPO CONTROL
	else if (s.equals("STCON")) {
	    STC = true;
	    tempoSlide.setEnabled(true);
	}
	else if (s.equals("STCOFF")) {
	    STC = false;
	    tempoSlide.setEnabled(false);
	}
	// LOCAL PATTERN CONTROL
	else if (s.equals("SPCON")) {
	    SPC = true;
	    for (i=0;i<rows;i++) {			
	...
	// FILE SELECT
	else if (s.equals("SFSON")) {
	    SFS = true;
	    for (i=0;i<rows;i++) {
		fileButtons[i].setEnabled(true);
	    }
	}
	else if (s.equals("SFSOFF")) {
	    SFS = false;
	    for (i=0;i<rows;i++) {
	...
	else if (s.equals("SBCON")) {
	    SBC = true;
	}
	else if (s.equals("SBCOFF")) {
	    SBC = false;
	}
	// INSTRUCTOR BUTTON CONTROL
	else if (s.equals("IBCON")) {
	    IBC = true;
	}
	else if (s.equals("IBCOFF")) {
	    IBC = false;
	}
