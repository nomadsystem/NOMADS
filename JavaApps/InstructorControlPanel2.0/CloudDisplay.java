...
public class CloudDisplay extends JPanel implements MouseListener, MouseMotionListener, ActionListener, Runnable {   
    ...
    int totCells = screenCellX * screenCellY;
    int cellTick = 0;
	...
	if (incAppID == NAppID.CLOUD_CHAT && incCmd == NCommand.SEND_MESSAGE) {
		...
		xRand = randNum.nextInt(xVar);
		yRand = randNum.nextInt(yVar);

		// curCell = cellSlots[cellCtr];
		curCellX = xRand + cellSlotsX[cellCtrX]; //pick x value from random array
		curCellY = yRand + cellSlotsY[cellCtrY]; //pick y value from random array
