...
public class NomadServer implements Runnable {  
	...
	if ((incAppID == NAppID.BINDLE) && (incAppCmd == NCommand.LOGIN)) {
		...
		// Log the client in
		// NB:  this is where we get the user's name
		//
	...
	if (incAppCmd != NCommand.REGISTER) {
	    ...
	    // TODO:  still need to handle other APP IDS
	    
	    // INSTRUCTOR_PANEL Routing Logic ==========================================================================R
		...
		for (int c = 0; c < clientCount; c++) {
		    ...
		    if (currentClient.getAppID() == NAppID.STUDENT_PANEL) {
		
			// if it's one of these commmands
			if ((incAppCmd == NCommand.SET_DISCUSS_STATUS) ||
			    (incAppCmd == NCommand.SET_CLOUD_STATUS) ||
			    (incAppCmd == NCommand.SET_POLL_STATUS) ||
			    (incAppCmd == NCommand.SET_SWARM_STATUS) ||
			    (incAppCmd == NCommand.SET_MOSAIC_STATUS) ||
			    (incAppCmd == NCommand.SET_UGROOVE_STATUS)) {
